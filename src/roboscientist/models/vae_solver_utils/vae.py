...
from roboscientist.models.vae_solver_utils import formula_utils, formula_config
...
class FormulaVARE(nn.Module):
    ...
    def reconstructed_formulas_from_encoded_formulas(self, encoded_formulas):
        ...
        for e_formula in encoded_formulas:
            reconstructed_formulas.append([formula_config.INDEX_TO_TOKEN[id] for id in e_formula[1:]])
        reconstructed_formulas = [
            f[:f.index(formula_config.END_OF_SEQUENCE)] \
                if formula_config.END_OF_SEQUENCE in f else f for f in reconstructed_formulas]
    ...
    def _reconstruct_encoded_formulas_from_latent(self, zs, max_len, explore=False, eps=0.2):
        ...
        x = torch.zeros(1, len(zs), dtype=torch.long, device=self.device).fill_(
            formula_config.TOKEN_TO_INDEX[formula_config.START_OF_SEQUENCE])
        # x: (1, z_in_batch)
        hidden = None
    ...
    def sample(self, n_formulas, max_len, out_file=None, ensure_valid=True, unique=True):
        ...
        if ensure_valid:
            ...
            for f in reconstructed_formulas:
                maybe_valid = formula_utils.maybe_get_valid(f)
