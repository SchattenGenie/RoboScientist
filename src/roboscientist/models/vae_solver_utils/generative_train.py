from roboscientist.models.vae_solver_utils import train, evaluate_formula, formula_utils, batch_builder
...
def _pretrain(n_pretrain_steps, model, optimizer, pretrain_batches, pretrain_val_batches, kl_coef):
    for step in range(n_pretrain_steps):
        train.run_epoch(model, optimizer, pretrain_batches, pretrain_val_batches, step, kl_coef)
    ...
    if np.isfinite(np.mean(sorted_best_mses)):
        ...
        if (epoch + 1) % 50 == 0:
            ...
            for f, m in zip(sorted_best_formulas[:20], sorted_best_mses[:20]):
                table.add_data(formula_utils.get_formula_representation(f.split()), str(m))
...
def generative_train(model, optimizer, epochs, device, batch_size,
    ...
    for epoch in range(epochs):
        ...
        mses, ress, coeffs, optimized_formulas = evaluate_formula.evaluate_file(file_to_sample, xs, ys)
        ...
        train_batches, _ = batch_builder.build_ordered_batches(retrain_file, batch_size, device)
        if not no_retrain:
            train.run_epoch(model, optimizer, train_batches, train_batches, epoch, kl_coef)
