...
from roboscientist.models.vae_solver_utils import formula_config, vae, batch_builder, generative_train, monitoring
...
def exp_generative_train(xs, ys, formula, train_file, val_file, max_len, epochs,
    ...
    train_monitoring = monitoring.Monitoring(project_name=project_name, correct_formula=formula,
                                          x_range=f'x_min={min(xs)}, x_max={max(xs)}, x_count={len(xs)}',
                                          experiment_config=experiment_config)

    device = torch.device('cuda')
    train_batches, _ = batch_builder.build_ordered_batches(formula_file=train_file, batch_size=batch_size,
                                                              device=device)
    valid_batches, _ = batch_builder.build_ordered_batches(formula_file=val_file, batch_size=batch_size,
                                                              device=device)
    model_params = vae.ModelParams(vocab_size=len(formula_config.INDEX_TO_TOKEN), device=device, **model_conf_params)
    model = vae.FormulaVARE(model_params)
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, betas=betas)
    generative_train.generative_train(model=model, optimizer=optimizer, epochs=epochs, device=device,
                                         ...
                                         use_n_last_steps=use_n_last_steps, monitoring=train_monitoring,
