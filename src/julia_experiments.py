...
def formula_for_active_learning_1_AL_MAX_VAE_16(exp_name):
    with open('wandb_key') as f:
        os.environ["WANDB_API_KEY"] = f.read().strip()
    f = equations_utils.infix_to_expr_with_arities(
        ['Add', 'cos', 'Div', 'Add', 'cos', "Symbol('x0')", "Symbol('x0')",
         ...
         "Symbol('x0')", "Symbol('x0')"],
        func_to_arity={'sin': 1, 'cos': 1, 'Add': 2, 'Mul': 2, 'Sub': 2, 'Div': 2, 'Pow': 2})
    f = equations_base.Equation(f, space=((0.1, 2.9),))
    X = np.linspace(0.1, 2.9, num=5000).reshape(-1, 1)
    f.add_observation(X)
    ...
    logger = single_formula_logger.SingleFormulaLogger('some_experiments',
                                                       exp_name + \
                                                       'formula_for_active_learning_1_AL_MAX_VAE_16',
                                                       logger_init_conf)
    vs = VAESolver(logger, 'checkpoint_div_sub_sin_cos_mul_add_no_constants', vae_solver_params)
    vs.solve(f, epochs=100)
...
def formula_for_active_learning_1_AL_RANDOM_16(exp_name):
    ...
    f = equations_utils.infix_to_expr_with_arities(
        ['Add', 'cos', 'Div', 'Add', 'cos', "Symbol('x0')", "Symbol('x0')",
         ...
         "Symbol('x0')", "Symbol('x0')"],
        func_to_arity={'sin': 1, 'cos': 1, 'Add': 2, 'Mul': 2, 'Sub': 2, 'Div': 2, 'Pow': 2})
    f = equations_base.Equation(f, space=((0.1, 2.95),))
    X = np.linspace(0.1, 2.95, num=5000).reshape(-1, 1)
    f.add_observation(X)
    X = np.array([1, 1.5, 2]).reshape(-1, 1)
    y_true = f.func(X)
    print(f)
